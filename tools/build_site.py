...
def build_checkout(cfg):
    return (head(cfg, f"Checkout — {cfg['brand']}", "Secure checkout.", "checkout.html", noindex=True)
            + header(cfg)
            + f"""<section class="pagehead"><div class="wrap">
  <nav class="crumbs"><a href="index.html">Home</a> / <a href="cart.html">Basket</a> / <span>Checkout</span></nav>
  <h1>Checkout</h1>
  <p class="muted">Step 1 of 2 &mdash; your details. Payment is taken on the next step by our
     payment provider; card details are never entered on or stored by this site.</p>
</div></section>
<section><div class="wrap cartlayout">
  <form class="form" id="checkoutForm" novalidate>
    <fieldset><legend>Contact</legend>
      <div class="field"><label for="ck-email">Email</label>
        <input id="ck-email" name="email" type="email" autocomplete="email" required></div>
      <div class="field"><label for="ck-phone">Phone (the carrier calls to book delivery)</label>
        <input id="ck-phone" name="phone" type="tel" autocomplete="tel" required></div>
    </fieldset>
    <fieldset><legend>Delivery address</legend>
      <div class="field"><label for="ck-name">Full name</label>
        <input id="ck-name" name="name" autocomplete="name" required></div>
      <div class="field"><label for="ck-addr">Address</label>
        <input id="ck-addr" name="address" autocomplete="street-address" required></div>
      <div class="field2">
        <div class="field"><label for="ck-city">Town or city</label>
          <input id="ck-city" name="city" autocomplete="address-level2" required></div>
        <div class="field"><label for="ck-pc">Postcode</label>
          <input id="ck-pc" name="postcode" autocomplete="postal-code" required></div>
      </div>
      <div class="field"><label for="ck-notes">Access notes (gradient, gate width, parking)</label>
        <textarea id="ck-notes" name="notes" rows="3"></textarea></div>
    </fieldset>
    <fieldset><legend>Confirm</legend>
      <label class="check"><input type="checkbox" id="ck-terms" required>
        I have read and accept the <a href="terms.html">terms &amp; conditions</a>,
        <a href="returns.html">returns policy</a> and <a href="privacy.html">privacy policy</a>.</label>
    </fieldset>
    <button class="btn btn--primary btn--lg btn--block" type="submit">Continue to payment</button>
    <p class="formnote">You will be taken to our payment provider to pay by card, PayPal or Apple Pay.
      Your order is not placed until payment completes.</p>
  </form>
  <aside class="summary">
    <h2>Order summary</h2>
    ...
    <dl>
      <div><dt>Subtotal (ex. VAT)</dt><dd data-sum-net>&pound;0</dd></div>
      <div><dt>VAT at 20%</dt><dd data-sum-vat>&pound;0</dd></div>
      <div><dt>Delivery</dt><dd>Free (UK mainland)</dd></div>
      <div class="total"><dt>Total to pay</dt><dd data-sum-total>&pound;0</dd></div>
    </dl>
    <ul class="minipolicy">
      <li><a href="returns.html">30-day returns</a></li>
...
<div class="result" id="ckResult" role="status"></div>""" + footer(cfg))
