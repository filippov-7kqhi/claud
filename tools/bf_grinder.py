...
def grinder(brand=True, guard=True, arm_deg=0):
    ...
    A('<g opacity=".5"><rect x="470" y="330" width="330" height="230" rx="12" fill="#0d2a1c"/>'
      '<path d="M500 356 L830 356 L800 328 L470 328 Z" fill="#0d2a1c"/></g>')
    ...
    # ---- main deck / body --------------------------------------------------
    A(f'<rect x="470" y="356" width="330" height="152" rx="12" fill="url(#bf)" stroke="#0c2418" stroke-width="3"/>')
    A(f'<path d="M470 356 L800 356 L770 326 L452 328 Z" fill="url(#bfTop)" stroke="#0c2418" stroke-width="3"/>')
    A(louvres(690, 384, 96, 96, n=6))
    A(f'<rect x="486" y="378" width="182" height="80" rx="8" fill="#0f3325" opacity=".4"/>')
    A(bolts([(482,368),(788,368),(482,496),(788,496)]))

    # ---- engine cowl on top ------------------------------------------------
    A(f'<rect x="556" y="268" width="228" height="92" rx="12" fill="url(#bf)" stroke="#0c2418" stroke-width="3"/>')
    A(f'<path d="M556 268 L784 268 L760 244 L532 244 Z" fill="url(#bfTop)" stroke="#0c2418" stroke-width="3"/>')
    ...
    A(f'<path d="M476 414 L476 492 L300 552 L268 512 Z" fill="url(#bf)" stroke="#0c2418" stroke-width="3"/>')
    A(f'<path d="M462 408 L488 408 L488 500 L462 500 Z" fill="#164a2f" stroke="#0c2418" stroke-width="2"/>')
    # belt guard along arm
    A(f'<path d="M470 424 L306 480 L294 512 L470 470 Z" fill="#1a5537" opacity=".8"/>')
    # cutter wheel
    cx, cy, r = 258, 528, 76
    ...
    A(f'<rect x="808" y="286" width="112" height="66" rx="9" fill="#12161b" stroke="#0c2418" stroke-width="2" transform="rotate(-8 808 286)"/>')
    A(f'<g transform="rotate(-8 808 286)"><circle cx="836" cy="308" r="10" fill="#dc2626"/>'
      f'<circle cx="864" cy="308" r="8" fill="{LIME}"/><rect x="822" y="326" width="86" height="9" rx="4" fill="#3d434b"/></g>')
    ...
    # ---- decals ------------------------------------------------------------
    if brand:
        A(decal(606, 420, BRAND, size=25, col="#ffffff", sub="GRINDMASTER 380 TX", subcol=LIME))
        A(decal(670, 316, "380", size=34, col=LIME))
    return "".join(g)
