...
from svgkit import *
from hc_dumper import PAINT, BRAND, AMBER, SUN, GRAPH
MODEL = "VANGUARD 850 SL"


def loader(brand=True, arm_deg=0):
    g=[]; A=g.append
    A('<g opacity=".5"><rect x="452" y="382" width="330" height="132" rx="12" fill="#7c4a09"/>'
      '<path d="M452 382 L782 382 L752 354 L422 354 Z" fill="#7c4a09"/></g>')
    ...
    # ---- main body ---------------------------------------------------------
    A(f'<rect x="452" y="382" width="330" height="132" rx="12" fill="url(#hc)" stroke="#7c4a09" stroke-width="3"/>')
    A(f'<path d="M452 382 L782 382 L752 354 L422 354 Z" fill="url(#hcTop)" stroke="#7c4a09" stroke-width="3"/>')
    A(louvres(694, 400, 74, 92, n=6))
    A(bolts([(464,394),(770,394),(464,502),(770,502)]))

    # ---- rear tower / operator station -------------------------------------
    A(f'<rect x="700" y="268" width="104" height="118" rx="12" fill="url(#hc)" stroke="#7c4a09" stroke-width="3"/>')
    A(f'<path d="M700 268 L804 268 L780 244 L676 244 Z" fill="url(#hcTop)" stroke="#7c4a09" stroke-width="3"/>')
    A(f'<rect x="714" y="282" width="76" height="50" rx="8" fill="#12161b" stroke="#171b21" stroke-width="2"/>')
    A(f'<rect x="724" y="292" width="54" height="8" rx="4" fill="{SUN}"/>')
    ...
    A(f'<path d="M770 448 L830 448 L830 500 L770 512 Z" fill="url(#hc)" stroke="#7c4a09" stroke-width="3"/>')
    ...
    A(f'<path d="M736 420 Q566 300 316 400" fill="none" stroke="#7c4a09" stroke-width="42" stroke-linecap="round"/>')
    A(f'<path d="M736 420 Q566 300 316 400" fill="none" stroke="url(#hc)" stroke-width="34" stroke-linecap="round"/>')
    A(f'<path d="M736 420 Q566 300 316 400" fill="none" stroke="#fcd34d" stroke-width="8" stroke-linecap="round" opacity=".35"/>')
    ...
    A(f'<path d="M314 366 L206 374 L172 480 Q244 506 316 480 Z" fill="url(#hc)" stroke="#7c4a09" stroke-width="3"/>')
    A(f'<path d="M232 388 L300 384 L300 406 L224 412 Z" fill="#b45309" opacity=".32"/>')
    ...
    # ---- decals ------------------------------------------------------------
    if brand:
        A(decal(578, 446, BRAND, size=25, col="#1a1206", sub=MODEL, subcol="#4a3410"))
        A(decal(752, 370, "850", size=24, col="#1a1206"))
    return "".join(g)
